...
class AuxiliaryHeadCIFAR(nn.Module):
  ...
  def __init__(self, C, num_classes):
    ...
    self.features = nn.Sequential(
      ...
      nn.AvgPool1d(5, stride=3, padding=0, count_include_pad=False),
...
class AuxiliaryHeadImageNet(nn.Module):
  ...
  def __init__(self, C, num_classes):
    ...
    self.features = nn.Sequential(
      nn.ReLU(inplace=True),
      nn.AvgPool1d(5, stride=2, padding=0, count_include_pad=False),
      nn.Conv1d(C, 128, 1, bias=False),
      nn.BatchNorm1d(128),
      nn.ReLU(inplace=True),
      nn.Conv1d(128, 768, 2, bias=False),
...
def main():
  ...
  model = NetworkCIFAR(1, 3, 10, False, genotype).to(device)
